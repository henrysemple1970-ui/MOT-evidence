...
async function captureLocation(){
  try{
    ...
    if(Capacitor.isNativePlatform()){
      ...
      p=await Geolocation.getCurrentPosition({enableHighAccuracy:true,timeout:12000,maximumAge:0});
    }else{
    ...
    S.coords={lat:p.coords.latitude,lon:p.coords.longitude,accuracy:p.coords.accuracy};
    status("gpsStatus","good",`GPS captured automatically • ±${Math.round(p.coords.accuracy)} m`);
    if($("homeGps")) $("homeGps").textContent="GPS ready";
  }catch(e){
    S.coords=null;
    status("gpsStatus","bad","Location permission is required before taking evidence photos.");
    if($("homeGps")) $("homeGps").textContent="Location permission required";
  }
  update();
}
async function processPhoto(file,n){
  if(!cleanReg($("reg").value)) return alert("Enter the registration first.");
  const out=await watermark(file,n);S.photos[n]=out.blob;
...
document.querySelectorAll("button[data-p]").forEach(b=>b.onclick=async()=>{
  ...
  if(!S.coords){
    ...
    if(!S.coords) return alert("GPS location is required for the photo watermark. Allow location access, then try again.");
  }
  if(Capacitor.isNativePlatform()){
    try{
      ...
      const file=new File([blob],`mot-photo-${n}.${photo.format||"jpeg"}`,{type:blob.type||"image/jpeg"});
      await processPhoto(file,n);
    }catch(e){
      if(String(e?.message||e).toLowerCase().includes("cancel")) return;
      alert("Camera could not be opened. Check camera permission and try again.");
    }
  }else $("p"+n).click();
});

...
let currentScreen="home",recoveryReady=false,reviewRetakePhoto=null;
...
const DEFAULT_GOOGLE_CLIENT_ID="266716734319-rjid1rtk8kkg80d8kv8ci4bs96qf6age.apps.googleusercontent.com";
function clientId(){
  const el=$("googleClientId");
  return String(el?.value || localStorage.getItem("motGoogleClientId") || DEFAULT_GOOGLE_CLIENT_ID).trim();
}
...
function renderReviewPhotos(){
  ...
  box.querySelectorAll("[data-retake]").forEach(b=>b.onclick=()=>{reviewRetakePhoto=Number(b.dataset.retake);showScreen("photo"+b.dataset.retake)});
}
...
for(let n=1;n<=5;n++){
  $("p"+n).onchange=async e=>{
    ...
    const destination=captureDestination(n,reviewRetakePhoto===n);
    reviewRetakePhoto=null;showScreen(destination);
  };
}

...
$("gps").onclick=async()=>{try{S.coords=await getFreshGps();status("gpsStatus","good",`GPS captured • ${S.coords.lat.toFixed(6)}, ${S.coords.lon.toFixed(6)} • ±${Math.round(S.coords.accuracy)} m`);update()}catch{status("gpsStatus","bad","Location permission not granted or a fresh GPS fix was unavailable.")}};
...
function resetTest(){clearPhotos();S.coords=null;S.motVehicle=null;S.latestTest=null;S.regChecked=false;$("reg").value="";$("confirmReg").checked=false;$("allowDuplicate").checked=false;$("motSummary").innerHTML="";$("regCheckStatus").className="status hidden";$("gpsStatus").className="status hidden"}
